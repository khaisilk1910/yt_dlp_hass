...
class YoutubeDlpManager:
    ...
    def _audio_source_selectors() -> tuple[str, ...]:
        """Prefer the best downloadable audio and fall back to muxed media.

        Avoid synthetic ``ba.N`` selectors. ``--check-formats`` makes yt-dlp
        skip unavailable candidates while this broad selector keeps a muxed
        source as the final audio-extraction fallback.
        """
