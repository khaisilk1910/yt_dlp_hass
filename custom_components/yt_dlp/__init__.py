...
def _favorite_keys(value: object) -> list[str]:
...
FAVORITES_PLAYBACK_SET_SCHEMA = vol.Schema(
    {
        vol.Required(ATTR_ONLINE_SELECTED): _favorite_keys,
        vol.Required(ATTR_OFFLINE_SELECTED): _favorite_keys,
...
FAVORITES_PLAYBACK_START_SCHEMA = vol.Schema(
    {
        ...
        vol.Required(ATTR_QUEUE): _favorite_keys,
        vol.Required(ATTR_MEDIA_PLAYERS): _media_player_entities,
        vol.Required(ATTR_REPEAT_MODE): vol.In(("off", "one", "all")),
...
def get_favorites_playback_controller(hass: HomeAssistant) -> FavoritesPlaybackController:
    """Return the persistent Favorites playback controller."""
    manager = _get_loaded_manager(hass)
    controller = getattr(manager, "favorites_playback", None)
...
async def async_setup(hass: HomeAssistant, config: ConfigType) -> bool:
    ...
    async def async_play(call: ServiceCall) -> ServiceResponse | None:
        ...
        try:
            # Direct Media Player playback replaces an active Favorites queue,
            # while preserving its checked selections until Favorites Stop.
            await get_favorites_playback_controller(hass).async_cancel_active(
    ...
    async def async_favorites_playback_get(call: ServiceCall) -> ServiceResponse:
        """Return synchronized Favorites selections and active queue state."""
    ...
    async def async_favorites_playback_set(call: ServiceCall) -> ServiceResponse:
        """Persist Favorites selections/repeat mode in Home Assistant storage."""
    ...
    async def async_favorites_playback_start(call: ServiceCall) -> ServiceResponse:
        """Start a backend-owned Favorites queue."""
        try:
    ...
    async def async_favorites_playback_skip(call: ServiceCall) -> ServiceResponse:
        """Skip within the backend-owned Favorites queue."""
        try:
    ...
    async def async_favorites_playback_stop(call: ServiceCall) -> ServiceResponse:
        """Stop Favorites playback and clear persisted checked selections."""
...
async def async_setup_entry(hass: HomeAssistant, entry: ConfigEntry) -> bool:
    ...
    # Persistent Favorites state is loaded in a background task, so storage or
    # an unavailable media library cannot block Home Assistant startup.
...
async def async_unload_entry(hass: HomeAssistant, entry: ConfigEntry) -> bool:
    ...
    if isinstance(manager, YoutubeDlpManager):
        ...
        if isinstance(favorites_playback, FavoritesPlaybackController):
            await favorites_playback.async_stop()
        playback = getattr(manager, "playback_manager", None)
        if isinstance(playback, PlaybackManager):
