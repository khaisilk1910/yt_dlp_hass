...
class YtDlpMediaCard extends HTMLElement {
  ...
  _render() {
    ...
    const mediaTitle = this._nowPlaying?.title || state?.attributes?.media_title || "Chọn nội dung để phát";
      ...
      <ha-card class="card ${isPlaying ? "is-playing" : ""}" style="${this._presetVars()}">
        ...
        <div class="surface">
          <header>
            ...
            ${activityStatus ? `<div class="activity-badge ${activityStatus.className}"><span class="activity-icon">${this._icon(activityStatus.icon)}</span><span class="activity-label">${this._escape(activityStatus.label)}</span></div>` : ""}
          </header>
  ...
  _styles() {
    return `
      ...
      *{box-sizing:border-box}button,input,select{font:inherit}button{color:inherit}
      ...
      header{justify-content:space-between;gap:12px;min-width:0}.brand{gap:12px;min-width:0}.brand>div:last-child{min-width:0}.brand-icon{width:44px;height:44px;flex:0 0 44px;border-radius:14px;display:grid;place-items:center;background:linear-gradient(135deg,var(--accent),var(--accent2));color:#fff;box-shadow:0 8px 24px color-mix(in srgb,var(--accent) 30%,transparent)}.brand-icon ha-icon{--mdc-icon-size:26px}.eyebrow{font-size:clamp(8px,2.2cqi,10px);font-weight:800;letter-spacing:.16em;color:var(--muted);white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.brand-title{font-size:clamp(15px,4cqi,18px);font-weight:800;letter-spacing:-.02em;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.activity-badge{display:flex;align-items:center;justify-content:center;flex:0 0 auto;height:32px;padding:0 10px 0 6px;gap:7px;border:1px solid var(--border-soft);border-radius:11px;background:color-mix(in srgb,var(--surface-soft) 92%,transparent);color:var(--muted);font-size:10px;font-weight:800;letter-spacing:.01em;line-height:1;box-shadow:0 4px 14px rgba(0,0,0,.07);backdrop-filter:blur(8px)}.activity-icon{width:21px;height:21px;flex:0 0 21px;border-radius:7px;display:grid;place-items:center;background:var(--surface);color:currentColor}.activity-icon ha-icon{--mdc-icon-size:14px}.activity-label{white-space:nowrap}.activity-badge.playing{color:#139766;border-color:color-mix(in srgb,#28b77c 24%,var(--border-soft));background:color-mix(in srgb,#28b77c 7%,var(--card-bg))}.activity-badge.playing .activity-icon{background:color-mix(in srgb,#28b77c 16%,var(--card-bg));color:#18a36e}.activity-badge.downloading{color:color-mix(in srgb,var(--accent) 82%,var(--text));border-color:color-mix(in srgb,var(--accent) 24%,var(--border-soft));background:color-mix(in srgb,var(--accent) 7%,var(--card-bg))}.activity-badge.downloading .activity-icon{background:color-mix(in srgb,var(--accent) 15%,var(--card-bg));color:color-mix(in srgb,var(--accent) 90%,var(--text))}.activity-badge.paused{color:#b17a15;border-color:color-mix(in srgb,#d89a2b 24%,var(--border-soft));background:color-mix(in srgb,#d89a2b 7%,var(--card-bg))}.activity-badge.paused .activity-icon{background:color-mix(in srgb,#d89a2b 15%,var(--card-bg));color:#bd8117}.activity-badge.downloading .activity-icon ha-icon{animation:downloadFloat 1.25s ease-in-out infinite}
