...
class YtDlpMediaCard extends HTMLElement {
  ...
  _downloadPercent(job) {
    ...
    if (Number.isFinite(direct)) return Math.max(0, Math.min(99.4, direct));

    const downloaded = Number(job.downloaded_bytes || 0);
    const total = Number(job.total_bytes || 0);
    if (total > 0) return Math.max(0, Math.min(99.4, (downloaded / total) * 100));

    // yt-dlp does not expose a reliable FFmpeg post-processing percentage.
    // Keep the UI below 100% until the backend confirms the whole job is done.
  ...
  _renderDownloadPanel() {
    ...
    const displayPercent = job?.status === "completed" ? 100 : Math.min(99, Math.floor(percent));
        ...
        ${active || job ? `
          <div id="downloadJobCard" class="job-card ${job?.status === "completed" ? "done" : job?.status === "error" ? "failed" : ""}">
            <div class="job-top">
              ...
              <b id="downloadJobPercent">${displayPercent}%</b>
            </div>
  ...
  async _pollDownloadJob(force = false) {
    ...
    const now = Date.now();
    if (!force && now - this._lastDownloadPoll < 850) return;
  ...
  _styles() {
    return `
      ...
      *{box-sizing:border-box}button,input,select{font:inherit}button{color:inherit}
      ...
      header{justify-content:space-between;gap:12px;min-width:0}.brand{gap:12px;min-width:0}.brand>div:last-child{min-width:0}.brand-icon{width:44px;height:44px;flex:0 0 44px;border-radius:14px;display:grid;place-items:center;background:linear-gradient(135deg,var(--accent),var(--accent2));color:#fff;box-shadow:0 8px 24px color-mix(in srgb,var(--accent) 30%,transparent)}.brand-icon ha-icon{--mdc-icon-size:26px}.eyebrow{font-size:clamp(8px,2.2cqi,10px);font-weight:800;letter-spacing:.16em;color:var(--muted);white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.brand-title{font-size:clamp(15px,4cqi,18px);font-weight:800;letter-spacing:-.02em;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.activity-badge{flex:0 0 auto;height:30px;padding:0 9px;display:inline-flex;align-items:center;justify-content:center;gap:5px;border:1px solid var(--border-soft);border-radius:10px;background:var(--surface-soft);font-size:10px;line-height:1;font-weight:800;letter-spacing:.01em;color:var(--muted);box-shadow:0 2px 8px rgba(0,0,0,.06)}.activity-badge ha-icon{width:14px;height:14px;flex:0 0 14px;--mdc-icon-size:14px}.activity-badge.playing{color:#149867;border-color:color-mix(in srgb,#28b77c 35%,var(--border-soft));background:color-mix(in srgb,#28b77c 9%,var(--card-bg))}.activity-badge.downloading{color:color-mix(in srgb,var(--accent) 84%,var(--text));border-color:color-mix(in srgb,var(--accent) 38%,var(--border-soft));background:color-mix(in srgb,var(--accent) 10%,var(--card-bg))}.activity-badge.paused{color:color-mix(in srgb,var(--muted) 85%,var(--text));border-color:var(--border-soft);background:var(--surface-soft)}
