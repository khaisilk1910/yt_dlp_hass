...
class YtDlpMediaCard extends HTMLElement {
  ...
  _renderDownloadPanel() {
    ...
    return `
      <section class="panel ${this._tab === "download" ? "active" : ""}" id="downloadPanel">
      ...
      <ha-card class="card ${isPlaying ? "is-playing" : ""}">
        ...
        <div class="surface">
          ...
          <nav class="tabs">
            ...
            <button data-tab="download" class="${this._tab === "download" ? "active" : ""}">${this._icon("download")} <span>Download</span>${this._isDownloadActive() ? `<i class="tab-pulse"></i>` : ""}</button>
          </nav>
  ...
  _styles() {
    return `
      ...
      *{box-sizing:border-box}button,input,select{font:inherit}button{color:inherit}
      ...
      header,.brand,.speaker-row,.field,.now-playing,.controls,.volume-row,.tabs,.url-box,.library-tools,.search-box,.library-meta,.track-row,.download-hero,.job-top,.job-metrics,.result-file,.download-notice{display:flex;align-items:center}
      ...
      @media(max-width:520px){.surface{padding:18px}.status{display:none}.now-playing{gap:16px}.art-wrap{width:94px;height:94px;flex-basis:94px;border-radius:23px}.art{border-radius:19px}.track-title{font-size:20px}.controls{gap:9px}.icon-btn{width:40px;height:40px}.play-btn{width:58px;height:58px}.library-list{max-height:270px}.track-size{display:none}.tabs button{font-size:11px;gap:5px}.tabs button span{max-width:62px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.option-grid{grid-template-columns:1fr}.download-hero span{display:none}}
      @media(max-width:380px){.tabs b{display:none}.tabs button span{display:none}.tabs button{height:40px}.tabs ha-icon{--mdc-icon-size:20px}}
